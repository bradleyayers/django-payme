...
from payme.utils import get_backend_settings
from payme.signals import return_urls_query
...
def return_from_gw(request, payment_id):
    ...
    if payment.status == 'failed':
        return redirect(urls['failure'])
    elif payment.status == 'paid':
        return redirect(urls['paid'])
    elif payment.status == 'partially_paid':
        try:
            return redirect(urls['partially_paid'])
        except KeyError:
            return redirect(urls['paid'])
    return direct_to_template(request, 'payme/base_return.html',
                              {'payment': payment})
